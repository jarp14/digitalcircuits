 ...
 * @generated
 */
public abstract class DigitalAbstractExpression {
	...
	*/
	protected void setStatus(int severity, String message, Throwable throwable) {
		String pluginID = DigitalDiagramEditorPlugin.ID;
		this.status = new Status(severity, pluginID, -1, (message != null) ? message : "", throwable); //$NON-NLS-1$
		if (!this.status.isOK()) {
			DigitalDiagramEditorPlugin.getInstance().logError("Expression problem:" + message + "body:" + body(), //$NON-NLS-1$//$NON-NLS-2$
					throwable);
		}
	}
	...
	* @generated
	*/
	protected DigitalAbstractExpression(String body, EClassifier context) {
		myBody = body;
		myContext = context;
	}
	...
	public Object evaluate(Object context, Map env) {
		if (context().isInstance(context)) {
			try {
				...
				DigitalDiagramEditorPlugin.getInstance().logError("Expression evaluation failure: " + body(), e); //$NON-NLS-1$
			}
		}
		return null;
	}
